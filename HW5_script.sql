...
CREATE VIEW chip_cars
AS SELECT *
FROM AUTO
WHERE PRICE_IN_DOLLARS < 25000;

SELECT * FROM chip_cars;

# Изменить в существующем представлении порог для стоимости: пусть цена будет до 30 000 долларов (используя оператор ALTER VIEW)
ALTER VIEW chip_cars
...
DROP VIEW only_skoda_and_audi;

CREATE VIEW only_skoda_and_audi
AS SELECT *
FROM AUTO
WHERE MARK='SKODA' OR MARK='AUDI';

SELECT * FROM only_skoda_and_audi;